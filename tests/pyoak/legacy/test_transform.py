...
import pytest
from pyoak.legacy.error import ASTTransformError
from pyoak.legacy.node import ASTTransformer, ASTTransformVisitor
from pyoak.legacy.node import AwareASTNode as ASTNode
from pyoak.origin import NoOrigin


@dataclass
class LegacyTRChildNode(ASTNode):
...
@dataclass
class LegacyTRParentNode(ASTNode):
    attr: str
    child: LegacyTRChildNode | None
    child_nodes: tuple[LegacyTRChildNode | LegacyTRParentNode, ...]
    origin: NoOrigin = field(default_factory=NoOrigin, init=False)


@dataclass
class LegacyTRRootNode(ASTNode):
    child: LegacyTRParentNode
    child_nodes: tuple[LegacyTRParentNode, ...]
    origin: NoOrigin = field(default_factory=NoOrigin, init=False)


def test_transform_visitor() -> None:
    class TRVisitor(ASTTransformVisitor):
        def visit_LegacyTRChildNode(self, node: LegacyTRChildNode) -> LegacyTRChildNode:
            return node.replace(attr="changed " + node.attr)

        def visit_LegacyTRParentNode(self, node: LegacyTRParentNode) -> LegacyTRParentNode:
            ...
            for tr_child, orig_child in zip(changes["child_nodes"], node.child_nodes):
                if not isinstance(orig_child, LegacyTRChildNode):
                    continue
    ...
    child_keep1 = LegacyTRChildNode("keep", "child_keep1")
    child_keep2 = LegacyTRChildNode("keep", "child_keep2")
    child_keep3 = LegacyTRChildNode("keep", "child_keep3")
    child_keep4 = LegacyTRChildNode("keep", "child_keep4")
    child_remove1 = LegacyTRChildNode("remove", "child_remove1")
    child_remove2 = LegacyTRChildNode("remove", "child_remove2")
    child_transform1 = LegacyTRChildNode("transform", "child_transform1")
    child_transform2 = LegacyTRChildNode("transform", "child_transform2")

    parent1 = LegacyTRParentNode(
        attr="parent1",
        child=None,
        child_nodes=(child_remove1, child_transform1, child_remove2),
    )
    parent2 = LegacyTRParentNode(
        attr="parent2",
        child=None,
        child_nodes=(child_keep1, child_keep2, child_transform2),
    )
    parent3 = LegacyTRParentNode(attr="parent3", child=child_keep4, child_nodes=(child_keep3,))

    orig_root = LegacyTRRootNode(child=parent1, child_nodes=(parent2, parent3))
    ...
    assert isinstance(new_root, LegacyTRRootNode)
...
def test_transform_visitor_error_recovery() -> None:
    class TRVisitor(ASTTransformVisitor):
        ...
        def visit_TRParentNode(self, node: LegacyTRParentNode) -> LegacyTRParentNode:
            changes: dict[str, Any] = {"attr": "changed " + node.attr}
            changes.update(self._transform_children(node))
            kept = 0
    ...
    orig_root = LegacyTRRootNode(child=parent1, child_nodes=(parent2,))
...
def test_transformer() -> None:
    class TRTransformer(ASTTransformer):
        def filter(self, node: ASTNode) -> bool:
            return isinstance(node, LegacyTRChildNode) and node.what in (
                "transform",
                "remove",
            )

        def transform(self, node: ASTNode) -> ASTNode | None:
            assert isinstance(node, LegacyTRChildNode)
    ...
    parent3 = LegacyTRParentNode(attr="parent3", child=None, child_nodes=(child_keep3,))

...
def test_get_xpath() -> None:
    n = TreeNested("test", origin=NO_ORIGIN)
    n_tree = n.to_tree()

    assert n_tree.get_xpath(n) == "/@root[0]TreeNested"

    m1 = TreeMiddle(n, origin=NO_ORIGIN)
    m1_tree = m1.to_tree()
    ...
    n2 = TreeNested("test2", origin=NO_ORIGIN)
    m2 = TreeMiddle(n2, origin=NO_ORIGIN)
    r = TreeRoot((m1, m2), origin=NO_ORIGIN)
    r_tree = r.to_tree()
...
def test_get_parent() -> None:
    n1 = TreeNested("test1", origin=NO_ORIGIN)
    m_m = TreeMiddleSubclass(n1, origin=NO_ORIGIN)
    m_r = TreeMiddle(m_m, origin=NO_ORIGIN)
    r = TreeRoot((m_r,), origin=NO_ORIGIN)
    r_tree = r.to_tree()
    ...
    with pytest.raises(KeyError):
        r_tree.get_parent(TreeOtherNode("test", origin=NO_ORIGIN))
...
def test_get_parent_info() -> None:
    ...
    with pytest.raises(KeyError):
        r_tree.get_parent_info(TreeOtherNode("test", origin=NO_ORIGIN))


def test_is_root() -> None:
    n = TreeNested("test1", origin=NO_ORIGIN)
...
def test_is_in_tree() -> None:
    ...
    n2 = TreeNested("test2", origin=NO_ORIGIN)
    m2 = TreeMiddle(n2, origin=NO_ORIGIN)
    r = TreeRoot((m_r, m2), origin=NO_ORIGIN)
    r_tree = r.to_tree()
    assert all(r_tree.is_in_tree(n) for n in (n1, m_m, m_r, n2, m2, r))
    assert not r_tree.is_in_tree(TreeOtherNode("test", origin=NO_ORIGIN))
...
def test_get_depth() -> None:
    ...
    with pytest.raises(KeyError):
        r_tree.get_depth(TreeOtherNode("test", origin=NO_ORIGIN))
...
def test_get_ancestors() -> None:
    ...
    with pytest.raises(KeyError):
        next(r_tree.get_ancestors(TreeOtherNode("test", origin=NO_ORIGIN)))

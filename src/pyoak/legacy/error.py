...
class ASTNodeRegistryCollisionError(ASTNodeError):
    """Raised when trying to create a new or attach a detached node to a
    registry, but there is already an existing ASTNode with the same ID."""
...
class ASTNodeDuplicateChildrenError(ASTNodeError):
    """Raised when an ASTNode has the same child more than once among
    children."""

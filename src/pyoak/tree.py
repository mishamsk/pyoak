...
class Tree:
    ...
    def get_depth(
        ...
        """Returns the depth of `node` in the tree either up to root or up to `relative_to` node (if
        it is the ancestor at all).
    ...
    def get_first_ancestor_of_type(
        ...
        *,
        ...
        """Returns the first ancestor of the `node` that is an instance of `ancestor_class`. Or None
    ...
    def is_ancestor(self, node: ASTNode, ancestor: ASTNode) -> bool:
        """Returns True if the `ancestor` is an ancestor of the `node` in the tree.
